...
const data = (function dummyData() { /* TODO replace with call to backend */
    return {
            ...
            meat: [
              'beef', 'chicken', 'ham', 'pork', 'ground beef', 'sausages'
            ],
            fish: ['tuna', 'salmon', 'talapia', 'perch', 'swordfish', 'cod'],
            fruit: ['banana', 'cherry', 'raspberry', 'apple', 'apricot', 'kiwi', 'orange', 'blueberry', 'blackberry', 'grapefruit', 'honeydew melon', 'cantoulope', 'grapes', 'plum' ],
            spices: ['pepper', 'salt', 'oregano', 'thyme', 'basil', 'nutmeg', 'allspice', 'cinnamon', 'ginger', 'bay leaf', 'marjoram', 'tumeric', 'sage', 'curry', 'vanilla', 'cumin', 'carraway', 'chili flakes']
          }
})()
...
const searchIngredients = (query, breadcrumbs = []) => {
    ...
    const matchSet = new Set(Object.keys(matches).map(container => {
        ...
        return [...matches[container].map((result => JSON.stringify(formatResult({...result, type}))))]
    }).flat())
