...
const Main = () => {
  ...
  const fetchRecipes = async () => {
    ...
    const data = Array.isArray(fetchedRecipes.current.data) ? [...fetchedRecipes.current?.data, ...recipes.data] : recipes.data
  ...
  const handleNext = async () => {
    if (fetchedRecipes.current.length - 1 <= activeRecipeIndex && fetchedRecipes.current.next !== "end") {
      fetchRecipes()
    }
    setActiveRecipeIndex(activeRecipeIndex + 1)
    setRecipeData(fetchedRecipes.current.data[activeRecipeIndex + 1])
